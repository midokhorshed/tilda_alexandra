 ...
 * tilda-phone-mask, tilda-calc, и тд.
 *
 ...
 *
 * li_rule - validation rule, used in one line input field
 ...
 * li_dateUnavailToday - unavailable Today
...
*  {
	...
	*  li_dateUnavailToday: 'y' | undefined,
   ...
   function t_zeroForms__init(recid, elemid, formObjectEls) {
	   ...
	   var isOldPage = document.querySelector('script[src*="tilda-blocks-2.7"]');
	   ...
	   t_onFuncLoad('t396_elem__renderViewOneField', function () {
		   t396_elem__renderViewOneField(isOldPage ? $(zeroForm) : zeroForm, 'left');
		   t396_elem__renderViewOneField(isOldPage ? $(zeroForm) : zeroForm, 'top');
	   });
   }
   ...
   function t_zeroForms__renderForm(zeroForm, formObjectEls, recid, tildamode) {
	   ...
	   if (!recid && window.tildamode !== 'zero' && window.tildamode !== 'edit') {
	   ...
	   if (typeof formObjectEls === 'string') {
		   try {
	   ...
	   // prettier-ignore
	   if ((t_zeroForms__isRecordHidden(rec) || t_zeroForms__isFormOutside(zeroForm)) && window.tildamode === 'published') return;
   ...
   function t_zeroForm__createForm(recid, tildamode, arrayOfElements, zeroForm) {
	   if (!tildamode) tildamode = t_zeroForms__getTildaMode();
	   ...
	   if (formObj.formbottomtext) {
		   ...
		   bottomText.appendChild(bottomBLock);
   ...
   function t_zeroForms__updateStylesOnResize(recid, zeroForm) {
	   ...
	   var formID = zeroForm.getAttribute('data-elem-id');
	   var inputFontWeight =
		   formObj.inputfontweight === 'variation' ? formObj.inputvariationweight : formObj.inputfontweight;
   
	   var phoneMaskInputs = document.querySelectorAll('.t-input-phonemask__wrap');
	   Array.prototype.forEach.call(phoneMaskInputs, function (phoneMaskInput) {
		   phoneMaskInput.classList.add('t-input-inline-styles');
	   });
   
	   // update inputs styles
	   var inputStyles = t_zeroForms__initInputStyles(formObj, inputFontWeight);
	   var inputs = zeroForm.querySelectorAll('.t-input-inline-styles');
	   Array.prototype.forEach.call(inputs, function (input) {
		   t_zeroForms__appendStylesToField(input, inputStyles);
		   // property resize for textarea
		   var rows = input.getAttribute('data-rows');
		   if (rows) {
			   var textAreaRowHeight = 25;
			   var textAreaPadding = 10;
			   input.style.height = rows * textAreaRowHeight + textAreaPadding + 'px';
		   }
	   });
   
	   // update title and subtitle styles
	   var titleStyles = {
		   color: formObj.inputtitlecolor || '',
		   fontWeight: formObj.inputtitlefontweight || '',
		   fontFamily: formObj.inputfontfamily || '',
	   ...
	   var titleFontWeight =
		   formObj.inputtitlefontweight === 'variation' ? formObj.inputtitlevariationweight : formObj.inputtitlefontweight;
	   var titleList = zeroForm.querySelectorAll('.t-input-title');
	   Array.prototype.forEach.call(titleList, function (title) {
		   t_zeroForms__setTitleStyles(title, titleStyles, titleFontWeight);
	   });
   
	   // update styles for input group
	   ...
	   // update submit btn styles
	   var submitBtn = zeroForm.querySelector('.t-submit');
	   if (!submitBtn) return;
	   // prettier-ignore
	   t_zeroForms__setBtnInlineStyles(formObj, submitBtn);
	   t_zeroForm__generateBtnStyles(submitBtn, recid, formID, formObj, zeroForm);
   
	   // update submit btn wrapper
	   var submitBtnWrapper = submitBtn.closest('.tn-form__submit');
	   if (!submitBtnWrapper) return;
	   submitBtnWrapper.style.textAlign = formObj.buttonalign;
	   submitBtnWrapper.style.marginTop = formObj.buttonmargtop + 'px';
   }
   ...
   function t_zeroForms__parseIntoElement(recid, zeroForm, elementObj, formObj, titleStyles, formID) {
	   ...
	   switch (elementObj.li_type) {
		   ...
		   case 'rd': // radio (question with answers)
			   ...
			   fieldSetStyles += 'fieldset {padding: 0; margin: 0; border: none;}';
			   t_zeroForms__setScriptOrStyle('t-zero-form-fieldset-' + recid, '', fieldSetStyles, 'style', zeroForm);
		   ...
		   case 'rg': // range slider (quantity)
			   // prettier-ignore
			   generatedField = t_zeroForms__createQuantityRange(elementObj, inputStyles, formObj, inputPreferences);
		   ...
		   case 'ws': // white space
			   ...
			   if (elementObj.li_rows) generatedField.style.height = elementObj.li_rows * WhiteSpaceRowHeight + 'px';
			   break;
		   case 'fr': // formula (calculation field)
			   ...
			   t_zeroForm__onRender(zeroForm, false, function () {
				   t_onFuncLoad('tcalc__init', function () {
					   // TODO check and set at start of t_zeroForms__parseIntoElement()
					   if (!recid) {
						   recid = zeroForm.closest('.r') ? zeroForm.closest('.r').id : '';
   ...
   function t_zeroForms__createPhoneInput(recid, elementObj, inputStyles, inputPreferences) {
	   ...
	   if (maskType === 'a') {
		   ...
		   if (elementObj.li_maskcountry) input.setAttribute('data-phonemask-maskcountry', elementObj.li_maskcountry);
   ...
   function t_zeroForms__createInput(elementObj, inputStyles, inputPreferences, type) {
	   ...
	   if (type === 'textarea') {
		   var textAreaStyleContent =
			   '.t396__elem .t-input-group_ta textarea.t-input {padding-top:10px; vertical-align: unset; resize: none;}';
		   t_zeroForms__setScriptOrStyle('zero-forms-textarea-styles', '', textAreaStyleContent, 'style', false);
   ...
   /**
	*
	* @param {FormObjectEl} elementObj
	* @param {HTMLTextAreaElement} textarea
	*/
   function t_zeroForms__setTextareaHeight(elementObj, textarea) {
	   var textAreaRowHeight = 25;
	   var textAreaPadding = 10;
	   if (elementObj.li_rows > 1) {
		   textarea.style.height = elementObj.li_rows * textAreaRowHeight + textAreaPadding + 'px';
		   textarea.setAttribute('data-rows', elementObj.li_rows);
	   }
	   textarea.rows = parseInt(elementObj.li_rows, 10) || 3;
   }
   ...
   function t_zeroForms__createRadioImage(elementObj, formObj, inputPreferences) {
	   ...
	   arrayOfOptions.forEach(function (imgOption, i) {
		   ...
		   if (!isCheckbox) input.name = elementObj.li_nm;
		   if (!isCheckbox) input.classList.add('js-tilda-rule');
		   if (selectedOption && selectedOption === i + 1) input.checked = true;
		   if (inputPreferences.require) input.setAttribute('data-tilda-req', '1');
		   label.insertAdjacentElement('beforeend', input);
   ...
   function t_zeroForms__createCheckbox(elementObj, labelStyles, formObj, inputPreferences) {
	   ...
	   labelText.innerHTML = elementObj.li_label;
   ...
   function t_zeroForms__createQuantityField(elementObj, inputStyles, color, inputPreferences) {
	   ...
	   var minusBtn = t_zeroForms__createQuantityBtn('minus', color);
	   var plusBtn = t_zeroForms__createQuantityBtn('plus', color);
	   var input = document.createElement('input');
   ...
   function t_zeroForms__createQuantityRange(elementObj, inputStyles, formObj, inputPreferences) {
	   ...
	   if (formObj.inputelscolor) input.setAttribute('data-range-color', formObj.inputelscolor);
	   ...
	   var maxValueField = t_zeroForms__createRangeField('max', formObj.inputtitlecolor, elementObj.li_vmax || '10');
   ...
   function t_zeroForms__createFormButton(recid, zeroForm, formID, formObj) {
	   ...
	   btnWrapper.style.marginTop = formObj.buttonmargtop + 'px';
	   ...
	   if (window.tildamode === 'zero' || window.tildamode === 'edit') {
		   // fix for correctly view submit btn in editor
		   if (window.tn && window.tn.curResolution < 640) btn.style.whiteSpace = 'normal';
		   t_zeroForm__generateBtnStyles(btn, recid, formID, formObj, zeroForm);
	   } else {
		   t_zeroForm__onRender(zeroForm, true, function () {
			   t_zeroForm__generateBtnStyles(btn, recid, formID, formObj, zeroForm);
		   });
   ...
   function t_zeroForm__generateBtnStyles(btn, recid, formID, formObj, zeroForm) {
	   ...
	   var btnStylesID = 't-zero-form-btn-styles-' + formID + '-' + recid;
	   var btnStyles = document.getElementById(btnStylesID);
	   if (btnStyles && btnStyles.parentElement) btnStyles.parentElement.removeChild(btnStyles);
	   t_zeroForms__setScriptOrStyle(btnStylesID, '', styleValue, 'style', zeroForm);
   }
   ...
   function t_zeroForms__getBottomText(formID, formObj, recid, zeroForm) {
	   ...
	   if (!isCheckbox) {
		   ...
		   span.innerHTML = formObj.formbottomtext;
		   return span;
	   } else {
		   ...
		   var labelText = document.createElement('span');
		   labelText.classList.add('t-checkbox__labeltext');
		   labelText.innerHTML = formObj.formbottomtext;
		   label.insertAdjacentElement('beforeend', labelText);
   ...
   function t_zeroForms__appendStylesToField(input, inputStyles) {
	   if (!input.classList.contains('t-input-inline-styles')) input.classList.add('t-input-inline-styles');
	   for (var k in inputStyles) {
		   input.style[k] = inputStyles[k];
	   }
	   if (!input.classList.contains('t-input-phonemask__wrap')) return;
	   var phoneMaskSelect = input.querySelector('.t-input-phonemask__select-code');
	   if (phoneMaskSelect) phoneMaskSelect.style.fontSize = inputStyles.fontSize;
	   if (phoneMaskSelect) phoneMaskSelect.style.fontWeight = inputStyles.fontWeight;
	   var phoneMaskInput = input.querySelector('.t-input-phonemask');
	   if (phoneMaskInput) phoneMaskInput.style.fontSize = inputStyles.fontSize;
	   if (phoneMaskInput) phoneMaskInput.style.fontWeight = inputStyles.fontWeight;
   }
   ...
   function t_zeroForms__createInputPlaceholderStyles(formID, formObj, recid, zeroForm) {
	   ...
	   var stylesID = 't-zero-placeholder-' + recid + '-' + formID;
	   t_zeroForms__setScriptOrStyle(stylesID, '', styles, 'style', zeroForm);
   }
   ...
   function t_zeroForms__setScriptOrStyle(id, path, content, type, zeroForm) {
	   ...
	   element.id = id;
	   var atomEl = type === 'style' && zeroForm ? zeroForm.querySelector('.tn-atom') : null;
	...
	*
	* @param {HTMLDivElement | HTMLLabelElement} textEl
	* @param titleStyles
   ...
   function t_zeroForms__setTitleStyles(textEl, titleStyles, titleFontWeight) {
	   ...
	   if (isTitle) textEl.style.fontSize = titleStyles.fontSize ? titleStyles.fontSize + 'px' : '';
	   if (isTitle && titleFontWeight) textEl.style.fontWeight = titleFontWeight;
   }
   ...
   /**
	* if form placed outsize zeroblock (left/right), don't render it.
	* Don't necessary use form bottom pos, because non-render form will have 0px height.
	*
	* For Safari top offset with zoom property calculated without scaled value.
	* To fix it need multiply top position with scale factor.
	*
	* @param {HTMLElement} form
	* @returns {boolean}
	*/
   function t_zeroForms__isFormOutside(form) {
	   ...
	   if (formPos.right < 0) return true;
   
	   var overflowValue = artBoard ? artBoard.getAttribute('data-artboard-ovrflw') : false;
	   var isOverflowNotHidden = overflowValue === 'auto' || overflowValue === 'visible';
	   if (isOverflowNotHidden) return false;
   
	   if (formPos.left > artBoardPos.right + window.pageXOffset) return true;
   
	   var isScaled = artBoard && artBoard.classList.contains('t396__artboard_scale');
	   if (!isScaled) isScaled = t_zeroForms__getFieldValue(artBoard, 'data-artboard', 'upscale') === 'window';
   
	   // don't calculate form bottom position,
	   // because it isn't rendered now (height equals 0)
	   var formTopPos = formPos.top;
	   var artBoardTopPos = Math.floor(artBoardPos.top + window.pageYOffset);
	   var artBoardBottomPos = Math.floor(artBoardPos.bottom + window.pageYOffset);
   
	   if (isScaled) {
		   var isOnlyScalable = Boolean(
		   ...
		   if (!isOnlyScalable) {
			   formTopPos *= window.tn_scale_factor || 1;
			   formTopPos = Math.round(formTopPos);
		   }
	   }
	   formTopPos += window.pageYOffset;
	   return formTopPos > artBoardBottomPos || formTopPos < artBoardTopPos;
   }
   
   function t_zeroForms__getFieldValue(element, attrName, prop) {
	   if (!element) return;
	   var resolution = window.tn.curResolution;
	   // TODO change to window.tn.screens
	   var breakpoints = [1200, 960, 640, 480, 320];
	   var postfix = attrName === 'data-field' ? '-value' : '';
	   var dataField;
   
	   if (typeof window.tn.curResolution === 'undefined') {
		   var rec = element.closest('.r');
		   var recid = rec ? rec.id.replace('rec', '') : '';
		   var artBoardId = 'ab' + recid;
   
		   resolution = window.tn[artBoardId].curResolution;
		   breakpoints = window.tn[artBoardId].screens;
	   }
   
	   breakpoints.forEach(function (breakpoint, i) {
		   if (i === 0 && +resolution >= breakpoint) {
			   dataField = element.getAttribute(attrName + '-' + prop + postfix);
		   }
		   if (i > 0 && +resolution === breakpoint) {
			   dataField = element.getAttribute(attrName + '-' + prop + '-res-' + breakpoint + postfix);
			   if (i > 1 && !dataField) {
				   var slicedBreakpoints = breakpoints.slice(1, i);
				   for (var n = slicedBreakpoints.length - 1; n >= 0; n--) {
					   dataField = element.getAttribute(attrName + '-' + prop + '-res-' + slicedBreakpoints[n] + postfix);
					   if (dataField) break;
				   }
			   }
			   if (!dataField) dataField = element.getAttribute(attrName + '-' + prop + postfix);
		   }
	   });
   ...
   function t_zeroFormsFromObjToArray(object) {
	   var isSupportedObjValues = typeof Object.values !== 'undefined';
	   var arrayOfOptions = isSupportedObjValues ? Object.values(object) : [];
	   if (!isSupportedObjValues) {
		   for (var k in object) {

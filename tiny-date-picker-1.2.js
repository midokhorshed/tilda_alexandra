...
function t_datepicker_init(recId, inputId, zeroElId) {
    ...
    if (zeroElId) {
        strSelectorId = '#rec' + recId + ' [data-elem-id="' + zeroElId + '"]' + ' [data-input-lid="' + inputId + '"]';
    } else {
    ...
    dateInput.setAttribute('autocomplete', navigator.userAgent.search(/Chrome/) > 0 ? 'no' : 'off');
    ...
    var options = {
        ...
        weekDays: weekDays,
        body: targetField,
        renderCallback: t_datepicker__renderCallback,
        renderValue: t_datepicker__renderValue,
        readValue: t_datepicker__readValue,
        sundayBased: false,
    };

    function t_datepicker__renderCallback(container) {
        var blocks = rec.querySelectorAll('.t-cover, .t396__artboard');

        container.style.cssText = !this.isOpen ? 'display: none' : 'display: block';

        if (blocks.length === 1) {
            if (this.isOpen) {
                blocks[0].style.overflow = 'unset';
            } else {
                blocks[0].style.overflow = '';
            }
        }
    }

    // eslint-disable-next-line no-undef
    new DatePicker(strSelectorId + ' .t-datepicker', options);
}

function t_datepicker__renderValue(container, element, value) {
    var pickedDate = value.split('-');
    var dateFormat = element.getAttribute('data-tilda-dateformat');
    var dateDiv = t_datepicker__getDateDiv(element);
    if (dateFormat === 'DD-MM-YYYY') {
        element.value = pickedDate[2] + dateDiv + pickedDate[1] + dateDiv + pickedDate[0];
    }
    if (dateFormat === 'MM-DD-YYYY') {
        element.value = pickedDate[1] + dateDiv + pickedDate[2] + dateDiv + pickedDate[0];
    }
    if (dateFormat === 'YYYY-MM-DD') {
        element.value = pickedDate[0] + dateDiv + pickedDate[1] + dateDiv + pickedDate[2];
    }
    if (element.classList.contains('t-input_pvis')) {
        element.classList.add('t-input_has-content');
    }
}

function t_datepicker__readValue(element) {
    var dateDiv = t_datepicker__getDateDiv(element);
    var curDate = element.value.split(dateDiv);
    var dateFormat = element.getAttribute('data-tilda-dateformat');
    if (curDate != '') {
        if (dateFormat === 'DD-MM-YYYY') {
            return (curDate[2] + '-' + curDate[1] + '-' + curDate[0]);
        }
        if (dateFormat === 'MM-DD-YYYY') {
            return (curDate[2] + '-' + curDate[0] + '-' + curDate[1]);
        }
        if (dateFormat === 'YYYY-MM-DD') {
...
function t_datepicker__getDateDiv(element) {
    var dateDivType = element.getAttribute('data-tilda-datediv');
    if (dateDivType === 'dash' || !dateDivType) {
        return '-';
    }
    if (dateDivType === 'slash') {
        return '/';
    }
    if (dateDivType === 'dot') {
        return '.';

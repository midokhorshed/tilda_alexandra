...
function t_serializeArray(form) {
	var arr = [];
	var elements = form.querySelectorAll('input, button, textarea, select');
...
(function () {
	...
	t_ready(function () {
		...
		window.tildaForm.paysystemRun = function (linkScript, systemName, form, functionCode, arArgs) {
			...
			/**
			 * TODO: this JQ-code can be removed when functions from server was
			 * moved from jQuery to JavaScript
			 */
		...
		window.tildaForm.sendStatAndShowMessage = function (form, objArgs, sendStat) {
			...
			for (var i = 0; i < inputTextarea.length; i++) {
				// Разву в textarea внутри может быть html код?
		...
		window.tildaForm.successEnd = function (form, successUrl, successCallback) {
			...
			for (var i = 0; i < inputTextarea.length; i++) {
				// Can there be html code inside the textarea?
				inputTextarea[i].innerHTML = '';
				inputTextarea[i].value = '';
			}
